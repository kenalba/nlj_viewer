...
class TestManageProfileUseCase:
    ...
    async def test_update_profile_info_success(self, manage_profile_use_case, current_user_context,
        ...
        mock_user_orm_service.update_user.return_value = updated_user
        ...
        with patch.object(manage_profile_use_case, '_publish_event') as mock_publish:
            ...
            mock_user_orm_service.update_user.assert_called_once()

            # Verify response structure
            assert isinstance(result, ManageProfileResponse)
            assert result.action_taken == ProfileAction.UPDATE_INFO
            assert isinstance(result.user, UserServiceSchema)
            assert result.user.full_name == "Updated Full Name"
            ...
            assert "updater_id" in event_kwargs
            assert "fields_updated" in event_kwargs
            assert "update_timestamp" in event_kwargs
            assert event_kwargs["user_id"] == current_user_context["user_id"]
            assert event_kwargs["updater_id"] == current_user_context["user_id"]
            assert "full_name" in event_kwargs["fields_updated"]
    ...
    async def test_change_password_success(self, manage_profile_use_case, current_user_context,
        ...
        mock_user_orm_service.get_by_id.return_value = mock_user_profile
        mock_user_orm_service.change_password.return_value = True
        ...
        with patch('app.core.security.verify_password', return_value=True):
            with patch.object(manage_profile_use_case, '_publish_event') as mock_publish:
                ...
                mock_user_orm_service.change_password.assert_called_once()
    ...
    async def test_setup_mfa_totp_success(self, manage_profile_use_case, current_user_context,
                                        mock_user_orm_service, mock_user_profile):
        """Test successful MFA setup with TOTP method."""
        # Setup
        mfa_request = ManageProfileRequest(
            action=ProfileAction.SETUP_MFA,
            mfa_method=MfaMethod.TOTP,
            mfa_code="123456"
        )

        mock_user_orm_service.get_by_id.return_value = mock_user_profile
        mock_user_orm_service.enable_mfa.return_value = True

        with patch.object(manage_profile_use_case, '_generate_mfa_secret', return_value="SECRET123"):
            with patch.object(manage_profile_use_case, '_validate_mfa_code', return_value=True):
                with patch.object(manage_profile_use_case, '_publish_event') as mock_publish:
                    # Execute
                    result = await manage_profile_use_case.execute(mfa_request, current_user_context)

                    # Verify MFA setup
                    assert result.action_taken == ProfileAction.SETUP_MFA
                    assert result.mfa_setup_data is not None
                    assert "secret" in result.mfa_setup_data
                    assert "qr_code_url" in result.mfa_setup_data

                    # Verify security event published
                    mock_publish.assert_called_once()
                    event_call = mock_publish.call_args
                    assert event_call[0][0] == "publish_user_mfa_enabled"
                    event_kwargs = event_call[1]
                    assert event_kwargs["mfa_method"] == "totp"
    ...
    async def test_update_preferences_success(self, manage_profile_use_case, current_user_context,
        ...
        with patch.object(manage_profile_use_case, '_publish_event') as mock_publish:
            ...
            mock_user_orm_service.update_user_preferences.assert_called_once()
            ...
            assert "preferences_updated" in event_kwargs
    ...
    async def test_upload_avatar_success(self, manage_profile_use_case, current_user_context,
        ...
        with patch.object(manage_profile_use_case, '_save_avatar_file', return_value="/avatars/user123.jpg"):
            with patch.object(manage_profile_use_case, '_publish_event') as mock_publish:
                # Execute
                ...
                mock_publish.assert_called_once()
                event_kwargs = mock_publish.call_args[1]
                assert "avatar_updated" in event_kwargs or "profile_updated" in event_kwargs[0][0]
    ...
    async def test_deactivate_account_success(self, manage_profile_use_case, current_user_context,
        ...
        mock_user_orm_service.get_by_id.return_value = mock_user_profile
        mock_user_orm_service.deactivate_user.return_value = True
        ...
        with patch.object(manage_profile_use_case, '_publish_event') as mock_publish:
            ...
            mock_publish.assert_called_once()
            event_call = mock_publish.call_args
            assert event_call[0][0] == "publish_user_account_deactivated"
            event_kwargs = event_call[1]
            assert event_kwargs["reason"] == "User requested account closure"
    ...
    async def test_admin_can_manage_other_users(self, manage_profile_use_case, admin_user_context,
        ...
        with patch.object(manage_profile_use_case, '_publish_event') as mock_publish:
            ...
            assert result.action_taken == ProfileAction.UPDATE_INFO
            assert result.user.full_name == "Admin Updated Name"

            # Verify event shows admin as updater
            event_kwargs = mock_publish.call_args[1]
            assert event_kwargs["updater_id"] == admin_user_context["user_id"]
            assert event_kwargs["user_id"] == str(target_user_id)
    ...
    async def test_non_admin_cannot_manage_other_users(self, manage_profile_use_case, current_user_context,
        ...
        with pytest.raises(PermissionError, match="manage other users"):
    ...
    async def test_event_driven_architecture_resilience(self, manage_profile_use_case, current_user_context,
        ...
        with patch.object(manage_profile_use_case, '_publish_event',
    ...
    @pytest.mark.parametrize("action,expected_event", [
        ...
        (ProfileAction.SETUP_MFA, "publish_user_mfa_enabled"),
        (ProfileAction.UPDATE_PREFERENCES, "publish_user_preferences_updated"),
    ])
    async def test_action_specific_xapi_events(self, manage_profile_use_case, current_user_context,
        ...
        if action == ProfileAction.UPDATE_INFO:
            ...
            with patch('app.core.security.verify_password', return_value=True):
                pass
        elif action == ProfileAction.SETUP_MFA:
            request = ManageProfileRequest(action=action, mfa_method=MfaMethod.TOTP, mfa_code="123456")
            mock_user_orm_service.enable_mfa.return_value = True
            with patch.object(manage_profile_use_case, '_generate_mfa_secret', return_value="SECRET123"):
                with patch.object(manage_profile_use_case, '_validate_mfa_code', return_value=True):
                    pass
