...
from sqlalchemy import String, Text, DateTime, ForeignKey, JSON, ARRAY
...
class ActivitySource(Base):
    ...
    citation_references: Mapped[List[str] | None] = mapped_column(
        ARRAY(String),
        comment="Specific page/section references used from the source document"
    )
    ...
    notes: Mapped[str | None] = mapped_column(
        Text,
        comment="Additional notes about how this source was used"
    )
    ...
    def __repr__(self) -> str:
        return f"<ActivitySource(activity_id={str(self.activity_id)[:8]}, source_id={str(self.source_document_id)[:8]})>"
    ...
    def get_citation_summary(self) -> str:
        ...
        return f"References: {', '.join(self.citation_references[:3])}{'...' if len(self.citation_references) > 3 else ''}"