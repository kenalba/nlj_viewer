...
from sqlalchemy import String, Text, DateTime, ForeignKey, JSON, Integer, Boolean, ARRAY
...
class MediaItem(Base):
    ...
    transcript: Mapped[str | None] = mapped_column(
        Text,
        comment="Generated transcript for audio/video content"
    )
    duration: Mapped[int | None] = mapped_column(
        Integer,
        comment="Duration in seconds for audio/video content"
    )
    
    # File storage
    file_path: Mapped[str | None] = mapped_column(
        String(500),
        comment="Local/cloud storage path for the media file"
    )
    ...
    mime_type: Mapped[str | None] = mapped_column(
        String(100),
        comment="MIME type (audio/mpeg, video/mp4, etc.)"
    )
    
    # Generation metadata
    generation_prompt: Mapped[str | None] = mapped_column(
        Text,
        comment="Original prompt used for content generation"
    )
    selected_keywords: Mapped[list[str]] = mapped_column(
        ARRAY(String),
        default=list,
        comment="Keywords selected for generation focus"
    )
    selected_objectives: Mapped[list[str]] = mapped_column(
        ARRAY(String),
        default=list,
        comment="Learning objectives used in generation"
    )
    generation_config: Mapped[dict[str, Any] | None] = mapped_column(
        JSON,
        comment="Generation settings (voice, style, length, etc.)"
    )
    
    # Media-specific configuration
    media_style: Mapped[MediaStyle | None] = mapped_column(
        String(30),
        comment="Style template used for generation"
    )
    voice_config: Mapped[dict[str, Any] | None] = mapped_column(
        JSON,
        comment="Voice settings for audio generation"
    )
    ...
    generation_end_time: Mapped[datetime | None] = mapped_column(
        DateTime(timezone=True),
        comment="When generation process completed"
    )
    ...
    play_count: Mapped[int] = mapped_column(
        Integer,
        default=0,
        comment="Number of times media has been played/viewed"
    )
    last_played_at: Mapped[datetime | None] = mapped_column(
        DateTime(timezone=True),
        comment="When media was last played/viewed"
    )
    ...
    is_public: Mapped[bool] = mapped_column(
        Boolean,
        default=False,
        comment="Whether media can be publicly shared"
    )
