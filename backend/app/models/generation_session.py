...
class GenerationSession(Base):
    ...
    claude_conversation_id: Mapped[str | None] = mapped_column(
        String(255), comment="Claude conversation ID for continuity"
    )
    claude_message_id: Mapped[str | None] = mapped_column(
        String(255), comment="Claude message ID for the generation request"
    )

    # Generation results
    generated_content: Mapped[dict[str, Any] | None] = mapped_column(JSON, comment="Raw response from Claude API")
    validated_nlj: Mapped[dict[str, Any] | None] = mapped_column(JSON, comment="Validated NLJ scenario data")
    validation_errors: Mapped[List[str] | None] = mapped_column(
        JSON, comment="Validation errors if NLJ schema validation failed"
    )

    # Generation metadata
    total_tokens_used: Mapped[int | None] = mapped_column(comment="Total tokens consumed in generation")
    generation_time_seconds: Mapped[float | None] = mapped_column(comment="Time taken for generation in seconds")
    ...
    # Relationships
    creator: Mapped["User"] = relationship(back_populates="generation_sessions", lazy="select")

    source_documents: Mapped[List["SourceDocument"]] = relationship(
        secondary=generation_session_sources, back_populates="generation_sessions", lazy="select"
    )

    created_activities: Mapped[List["ContentItem"]] = relationship(back_populates="generation_session", lazy="select")
    ...
    def complete_successfully(
        self, generated_content: dict, validated_nlj: dict, tokens_used: int = None, generation_time: float = None
    ) -> None:
