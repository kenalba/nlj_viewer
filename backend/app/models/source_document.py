...
from sqlalchemy import String, Text, DateTime, ForeignKey, JSON, BigInteger, Boolean, ARRAY
...
class SourceDocument(Base):
    ...
    filename: Mapped[str] = mapped_column(
        String(255),
        nullable=False,
        comment="Current filename (may be converted)"
    )
    ...
    file_type: Mapped[FileType] = mapped_column(
        String(20),
        nullable=False,
        comment="Current file type (after conversion if applicable)"
    )
    original_file_type: Mapped[FileType] = mapped_column(
        String(20),
        nullable=False,
        comment="Original file type before conversion"
    )
    ...
    claude_file_id: Mapped[str | None] = mapped_column(
        ...
        unique=True,
        index=True,
        comment="Claude Files API file ID"
    )
    ...
    # Content metadata
    extracted_title: Mapped[str | None] = mapped_column(
        String(500),
        comment="Title extracted from document metadata"
    )
    extracted_author: Mapped[str | None] = mapped_column(
        String(255),
        comment="Author extracted from document metadata"
    )
    ...
    keywords: Mapped[List[str] | None] = mapped_column(
        ARRAY(String),
        comment="AI-extracted keywords and key terms"
    )
    learning_objectives: Mapped[List[str] | None] = mapped_column(
        ARRAY(String),
        comment="Potential learning objectives derived from content"
    )
    content_type_classification: Mapped[str | None] = mapped_column(
        String(100),
        comment="AI-classified content type (training, reference, process, etc.)"
    )
    difficulty_level: Mapped[str | None] = mapped_column(
        String(20),
        comment="Content difficulty level (beginner, intermediate, advanced)"
    )
    estimated_reading_time: Mapped[int | None] = mapped_column(
        comment="Estimated reading time in minutes"
    )
    key_concepts: Mapped[List[str] | None] = mapped_column(
        ARRAY(String),
        comment="Main concepts that could become quiz questions"
    )
    target_audience: Mapped[str | None] = mapped_column(
        String(200),
        comment="Intended audience for this content"
    )
    subject_matter_areas: Mapped[List[str] | None] = mapped_column(
        ARRAY(String),
        comment="Subject areas covered (sales, technical, compliance, etc.)"
    )
    actionable_items: Mapped[List[str] | None] = mapped_column(
        ARRAY(String),
        comment="Specific steps/processes that could become scenarios"
    )
    assessment_opportunities: Mapped[List[str] | None] = mapped_column(
        ARRAY(String),
        comment="What could be tested or evaluated from this content"
    )
    content_gaps: Mapped[List[str] | None] = mapped_column(
        ARRAY(String),
        comment="What additional information might be needed"
    )
    ...
    expires_at: Mapped[datetime | None] = mapped_column(
        ...
        comment="Claude Files API expiration date"
    )
