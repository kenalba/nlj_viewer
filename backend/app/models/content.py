...
class ContentItem(Base):
    ...
    # Generation session that created this content (for AI-generated content)
    generation_session_id: Mapped[uuid.UUID | None] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("generation_sessions.id"),
        index=True,
        comment="Generation session that created this content",
    )

    # Template information
    is_template: Mapped[bool] = mapped_column(default=False)
    template_category: Mapped[str | None] = mapped_column(String(100), comment="Category for template organization")
    ...
    creator: Mapped["User"] = relationship(back_populates="created_content", lazy="select")

    # Self-referential relationship for content variants
    parent_content: Mapped["ContentItem | None"] = relationship(remote_side=[id], lazy="select")
    ...
    generation_session: Mapped["GenerationSession | None"] = relationship(
        back_populates="created_activities", lazy="select"
    )
    ...
    def get_source_documents(self) -> list:
        """Get list of source documents used to generate this content."""
        if not self.source_lineage:
            return []
        return [lineage.source_document for lineage in self.source_lineage]

    def get_source_summary(self) -> str:
        """Get a summary of source documents for display."""
        sources = self.get_source_documents()
        if not sources:
