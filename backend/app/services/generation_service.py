...
class GenerationService:
        ...
        try:
            ...
            for display_name, node_type, json_str in node_sections:
                try:
                    ...
                    examples[node_type] = {"displayName": display_name.strip(), "example": schema_example}
    ...
    async def create_generation_session(
        ...
        # Create session
        session = GenerationSession(user_id=user_id, prompt_config=prompt_config, status=GenerationStatus.PENDING)
    ...
    async def start_generation(self, session_id: uuid.UUID, user_id: uuid.UUID) -> bool:
        ...
        query = (
            ...
            .options(joinedload(GenerationSession.source_documents))
            .where(
                and_(
        ...
        if not session:
            ...
            debug_query = select(GenerationSession).where(
                and_(GenerationSession.id == session_id, GenerationSession.user_id == user_id)
            )
        ...
        try:
            ...
            for i, source_doc in enumerate(session.source_documents):
                ...
                if await self.source_service.ensure_claude_upload(source_doc):
                    ...
                    print(
                        f"✅ Document {source_doc.original_filename} uploaded, Claude file ID: {source_doc.claude_file_id}"
                    )
            ...
            content_info = (
                list(generated_content.keys()) if isinstance(generated_content, dict) else type(generated_content)
            )
            ...
            if "generated_json" in generated_content:
                is_valid, errors = await claude_service.validate_nlj_schema(generated_content["generated_json"])
                if is_valid:
    ...
    def _build_prompt_from_config(self, config: Dict[str, Any]) -> str:
        ...
        if include_variables or include_branching:
            prompt_parts.extend(["### Advanced Features:", ""])
    ...
    def _generate_schema_examples(
        self, enabled_types: List[str], include_variables: bool = False, include_branching: bool = False
    ) -> List[str]:
        ...
        question_types = [
            t for t in enabled_types if t in ["question", "true_false", "ordering", "matching", "short_answer"]
        ]
        other_types = [
            t
            for t in enabled_types
            if t not in question_types and t not in ["start", "end", "interstitial_panel", "branch"]
        ]
        ...
        for i, node_type in enumerate(types_to_show):
            ...
            if previous_node_id:
                links.append(
                    f'    {{ "id": "{previous_node_id}-to-{node_id}", "type": "link", "sourceNodeId": "{previous_node_id}", "targetNodeId": "{node_id}" }}'
                )
            ...
            if node_type == "question":
                ...
                node_examples.extend(
                    [
                        ...
                        '      "text": "'
                        + (
                            "Your current score is {score} points. Sample multiple choice question?"
                            if include_variables
                            else "Sample multiple choice question?"
                        )
                        + '",',
                        ...
                        '      "variableChanges": '
                        + ('[{"variableId": "score", "operation": "add", "value": 10}]' if include_variables else "[]")
                        + ",",
                        ...
                        '      "variableChanges": '
                        + (
                            '[{"variableId": "score", "operation": "subtract", "value": 5}]'
                            if include_variables
                            else "[]"
                        )
                        + ",",
                ...
                node_examples.extend(["    {", example_str, "    },"])
                ...
                links.append(
                    f'    {{ "id": "{node_id}-to-{next_node_id}", "type": "link", "sourceNodeId": "{node_id}", "targetNodeId": "{next_node_id}" }}'
                )
    ...
    async def get_user_sessions(
        self, user_id: uuid.UUID, status: Optional[GenerationStatus] = None, limit: int = 50, offset: int = 0
    ...
    async def get_session_by_id(self, session_id: uuid.UUID, user_id: uuid.UUID) -> Optional[GenerationSession]:
        """Get generation session by ID with ownership validation."""
        query = (
            select(GenerationSession)
            .options(joinedload(GenerationSession.source_documents), joinedload(GenerationSession.created_activities))
            .where(and_(GenerationSession.id == session_id, GenerationSession.user_id == user_id))
        )
    ...
    async def create_activity_from_session(
        self, session_id: uuid.UUID, user_id: uuid.UUID, title: str, description: Optional[str] = None
        ...
        try:
            ...
            content_data = ContentCreate(
                title=title, description=description, content_type=content_type, nlj_data=session.validated_nlj
            )
            ...
            for source_doc in session.source_documents:
                lineage = ActivitySource(
                    activity_id=activity.id, source_document_id=source_doc.id, generation_session_id=session.id
                )
                self.db.add(lineage)
