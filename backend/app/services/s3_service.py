...
class S3MediaService:
    ...
    async def upload_media_file(
        self, file: UploadFile, media_id: uuid.UUID, folder: str = "uploads", make_public: bool = False
    ) -> Dict[str, Any]:
        """
    ...
    async def download_media_file(self, s3_key: str, as_attachment: bool = True) -> tuple[bytes, str, str]:
        """
    ...
    def generate_presigned_url(self, s3_key: str, expires_in: int = 3600, method: str = "get_object") -> str:
        ...
        try:
            url = self.s3_client.generate_presigned_url(
                method, Params={"Bucket": self.media_bucket, "Key": s3_key}, ExpiresIn=expires_in
            )
            return url
    ...
    async def list_media_files(self, prefix: str = "", max_keys: int = 100) -> list[Dict[str, Any]]:
        """
        ...
        """
        try:
            response = self.s3_client.list_objects_v2(Bucket=self.media_bucket, Prefix=prefix, MaxKeys=max_keys)

            files = []
            for obj in response.get("Contents", []):
                # Get object metadata
                try:
                    head_response = self.s3_client.head_object(Bucket=self.media_bucket, Key=obj["Key"])
    ...
    async def backup_media_file(self, s3_key: str) -> bool:
        ...
        try:
            ...
            self.s3_client.copy_object(
                CopySource={"Bucket": self.media_bucket, "Key": s3_key}, Bucket=self.backup_bucket, Key=backup_key
            )
