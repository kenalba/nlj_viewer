...
from app.models.training_program import TrainingBooking, TrainingSession
...
class SchedulingService:
    ...
    async def find_scheduling_conflicts(
        ...
        conditions = [
            ...
            or_(
                ...
                and_(TrainingSession.start_time <= start_time, TrainingSession.end_time > start_time),
                # New session ends during existing session
                and_(TrainingSession.start_time < end_time, TrainingSession.end_time >= end_time),
                # New session completely contains existing session
                and_(TrainingSession.start_time >= start_time, TrainingSession.end_time <= end_time),
            ),
        ]
        ...
        stmt = select(TrainingSession).options(joinedload(TrainingSession.session)).where(and_(*conditions))
    ...
    async def create_training_instance(
        ...
        # Check for conflicts
        conflicts = await self.find_scheduling_conflicts(start_time, end_time, actual_location or session.location)
    ...
    async def update_training_instance(
        ...
        if start_time or end_time:
            ...
            conflicts = await self.find_scheduling_conflicts(
                instance.start_time, instance.end_time, instance.actual_location, exclude_instance_id=instance.id
            )
    ...
    async def cancel_training_instance(
        self, instance: TrainingSession, cancellation_reason: Optional[str] = None
    ) -> TrainingSession:
        ...
        stmt = select(TrainingBooking).where(
            and_(
                TrainingBooking.instance_id == instance.id, TrainingBooking.booking_status.in_(["confirmed", "pending"])
            )
        )
        result = await self.db.execute(stmt)
    ...
    async def create_booking(
        ...
        if existing_booking:
            raise SchedulingError("Learner already has a booking for this session", error_code="DUPLICATE_BOOKING")
        ...
        if current_bookings >= max_capacity:
            if session.allow_waitlist:
                ...
                waitlist_stmt = select(func.count(TrainingBooking.id)).where(
                    and_(TrainingBooking.instance_id == instance.id, TrainingBooking.booking_status == "waitlist")
                )
                waitlist_result = await self.db.execute(waitlist_stmt)
                waitlist_position = (waitlist_result.scalar() or 0) + 1
            else:
                raise SchedulingError("Training session is full and waitlist is not allowed", error_code="SESSION_FULL")
        else:
    ...
    async def get_availability_summary(
        self, session: TrainingSession, start_date: Optional[datetime] = None, end_date: Optional[datetime] = None
    ) -> Dict:
        ...
        for instance in instances:
            confirmed_bookings = len([b for b in instance.bookings if b.booking_status in ["confirmed", "pending"]])
            waitlist_bookings = len([b for b in instance.bookings if b.booking_status == "waitlist"])
        ...
        return {
            ...
            "upcoming_instances": len(
                [i for i in instances if i.start_time > datetime.now(timezone.utc) and i.status == "scheduled"]
            ),
