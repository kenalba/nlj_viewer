...
class WorkflowService:
    ...
    async def get_workflow_templates(
        self,
        content_type: WorkflowTemplateType | None = None,
        is_active: bool = True
    ...
    async def delegate_reviewer_assignment(
        ...
        result = await self.db.execute(
            select(StageReviewerAssignment)
            .where(StageReviewerAssignment.id == assignment_id)
        )
    ...
    async def _auto_assign_stage_reviewers(self, stage_instance: WorkflowStageInstance):
        ...
        if template_stage.auto_assign_to_role:
            ...
            result = await self.db.execute(
                select(User).where(User.role == template_stage.auto_assign_to_role)
            )
    ...
    async def get_stage_instances_for_reviewer(
        ...
        query = select(WorkflowStageInstance).options(
            ...
            StageReviewerAssignment.is_active == True,
            StageReviewerAssignment.has_reviewed == False
        )
