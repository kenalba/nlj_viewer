...
from app.models.source_document import ConversionStatus, FileType, SourceDocument
...
class SourceDocumentService:
    ...
    async def create_source_document(
        ...
        source_doc = SourceDocument(
            ...
            original_file_type=(
                FileType(original_file_type) if original_file_type in [ft.value for ft in FileType] else FileType.PDF
            ),
    ...
    async def _convert_document(self, source_doc: SourceDocument) -> bool:
        ...
        try:
            ...
            converted_path = await document_converter.convert_to_pdf(source_doc.file_path, file_type_str)
    ...
    async def upload_to_claude(self, source_doc: SourceDocument) -> bool:
        ...
        try:
            ...
            # Upload to Claude
            file_object = await claude_service.upload_file(source_doc.file_path, source_doc.original_filename)
    ...
    async def get_document_by_id(self, document_id: uuid.UUID, user_id: uuid.UUID) -> Optional[SourceDocument]:
        ...
        query = select(SourceDocument).where(and_(SourceDocument.id == document_id, SourceDocument.user_id == user_id))
        result = await self.db.execute(query)
    ...
    async def get_popular_documents(self, user_id: uuid.UUID, limit: int = 10) -> List[SourceDocument]:
        ...
        query = (
            ...
            .order_by(SourceDocument.usage_count.desc(), SourceDocument.last_used_at.desc())
            .limit(limit)
        )
    ...
    async def get_recent_documents(self, user_id: uuid.UUID, limit: int = 10) -> List[SourceDocument]:
        ...
        query = (
            select(SourceDocument)
            .where(and_(SourceDocument.user_id == user_id, SourceDocument.last_used_at.is_not(None)))
            .order_by(SourceDocument.last_used_at.desc())
            .limit(limit)
        )
    ...
    async def cleanup_expired_uploads(self) -> int:
        ...
        query = select(SourceDocument).where(
            and_(SourceDocument.expires_at.is_not(None), SourceDocument.expires_at < datetime.now(timezone.utc))
        )
...
Analyze the document thoroughly and provide only valid JSON with the fields you can determine. Focus on making this metadata useful for creating training content and learning activities."""
            ...
            response = await claude_service.generate_content_with_file_and_prefill(
                file_id=document.claude_file_id, prompt=prompt, prefill="{"
            )
                ...
                try:
                    ...
                    if json_match:
                        try:
                            ...
                            print("Found JSON in markdown block, attempting to parse...")
