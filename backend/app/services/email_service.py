...
class SESEmailService:
    ...
    async def send_survey_invitation(
        ...
        try:
            ...
            return await self._send_bulk_email(template=template, recipients=recipients, email_type="survey_invitation")

        except Exception as e:
            logger.error(f"Failed to send survey invitations: {e}")
            return EmailSendResult(
                success=False, recipient_count=len(recipients), error_message=str(e), timestamp=datetime.utcnow()
            )
    ...
    async def send_training_notification(
        ...
        try:
            ...
            return await self._send_bulk_email(
                template=template, recipients=recipients, email_type="training_notification"
            )
    ...
    async def send_admin_notification(
        ...
        try:
            ...
            return await self._send_bulk_email(
                template=template, recipients=[admin_recipient], email_type="admin_notification"
            )

        except Exception as e:
            logger.error(f"Failed to send admin notification: {e}")
            return EmailSendResult(success=False, recipient_count=1, error_message=str(e), timestamp=datetime.utcnow())

    async def _send_bulk_email(
        self, template: EmailTemplate, recipients: List[EmailRecipient], email_type: str
    ) -> EmailSendResult:
        """
    ...
    def _create_survey_invitation_template(
        ...
        return EmailTemplate(subject=subject, html_body=html_body, text_body=text_body, sender_name=sender_name)
    ...
    def _create_admin_html_template(
        self, message: str, notification_type: str, additional_data: Optional[Dict[str, Any]]
    ) -> str:
    ...
    def _create_admin_text_template(
        ...
        text = "NLJ Platform Admin Notification\n"
