    ...
    async def _can_change_state(
        ...
        if content.created_by == user_id:
            if user_role == UserRole.CREATOR:
                ...
                return (
                    (content.state == ContentState.DRAFT and 
                     new_state == ContentState.SUBMITTED) or
                    (content.state == ContentState.REJECTED and 
                     new_state == ContentState.DRAFT)
                )
        ...
        if user_role == UserRole.REVIEWER:
            ...
            return (
                content.state == ContentState.SUBMITTED and
                new_state in [ContentState.IN_REVIEW, ContentState.REJECTED]
            )
        ...
        if user_role == UserRole.APPROVER:
            ...
            return (
                (content.state == ContentState.IN_REVIEW and
                 new_state in [ContentState.APPROVED, ContentState.REJECTED]) or
                (content.state == ContentState.APPROVED and
