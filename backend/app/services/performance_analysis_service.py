...
class PerformanceAnalysisService:
    ...
    async def identify_top_performers(
        ...
        try:
            ...
            performance_data = await self._get_learner_performance_data(
    ...
    async def generate_training_recommendations(
        ...
        try:
            ...
            user_data = await self._get_user_detailed_data(user_id, 
                self._calculate_since_date("90d").isoformat())
    ...
    async def predict_compliance_risk(
        ...
        try:
            ...
            user_data = await self._get_user_detailed_data(user_id, 
                self._calculate_since_date("180d").isoformat())  # Longer history for better prediction
    ...
    async def _get_learner_performance_data(
        self, 
        since: str, 
        category: Optional[str] = None
    ) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if client:
                query = {
                    "size": 1,
                    "query": {
                        "term": {"actor.mbox": f"mailto:{email}"}
                    },
                    "_source": ["actor.name"]
                }
    ...
    async def _calculate_performance_trend(self, trend_buckets: List[Dict]) -> str:
        ...
        scores = [bucket.get("avg_score", {}).get("value", 0) for bucket in sorted_buckets if bucket.get("avg_score", {}).get("value")]
    ...
    async def _get_user_detailed_data(self, user_id: str, since: str) -> Dict[str, Any]:
        ...
        try:
            ...
            statements = await self.ralph_service.get_learner_statements(
                user_email, since=since, limit=500
            )
...
def get_performance_analysis_service() -> PerformanceAnalysisService:
    ...
    return PerformanceAnalysisService(
        ...
        ralph_service=ralph_lrs_service
    )