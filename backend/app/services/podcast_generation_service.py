...
class PodcastGenerationService:
    ...
    async def generate_podcast_async(
        ...
        try:
            ...
            source_document = await source_doc_service.get_document_by_id(
                media_item.source_document_id, 
                media_item.created_by
            )
            
            if not source_document:
                await media_service.mark_generation_failed(
                    db, media_id, "Source document not found"
                )
                return
            ...
            if not script:
                await media_service.mark_generation_failed(
                    db, media_id, "Failed to generate script"
                )
                return
    ...
    async def _generate_npr_script(
        ...
        script = await claude_service.generate_content_with_file(
            file_id=source_document.claude_file_id,
            prompt=prompt,
            max_tokens=4096,
            temperature=0.3
        )
    ...
    def _build_script_generation_prompt(
        ...
        objectives_text = "\n".join(f"- {obj}" for obj in selected_objectives) if selected_objectives else "- Provide informative overview\n- Engage listeners with key insights"
