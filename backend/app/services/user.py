...
class UserService:
    ...
    async def get_user_by_id(self, user_id: uuid.UUID) -> User | None:
        """Get user by ID."""
        result = await self.db.execute(select(User).options(joinedload(User.created_content)).where(User.id == user_id))
    ...
    async def get_user_by_username_or_email(self, identifier: str) -> User | None:
        ...
        result = await self.db.execute(select(User).where((User.username == identifier) | (User.email == identifier)))
        return result.scalar_one_or_none()
    ...
    async def update_user(self, user_id: uuid.UUID, user_update: UserUpdate) -> User | None:
        """
    ...
    async def get_user_count(
        self, role_filter: UserRole | None = None, active_only: bool = False, search: str | None = None
    ) -> int:
