...
class ContentBase(BaseModel):
    ...
    content_type: ContentType = Field(default=ContentType.TRAINING, description="Type of content")
    learning_style: Optional[LearningStyle] = Field(None, description="Primary learning style for this content")
    is_template: bool = Field(default=False, description="Whether this content is a template")
    template_category: Optional[str] = Field(None, max_length=100, description="Category for template organization")


class ContentCreate(ContentBase):
    """Schema for creating new content."""

    nlj_data: Dict[str, Any] = Field(..., description="Complete NLJ scenario data as JSON")
    parent_content_id: Optional[uuid.UUID] = Field(None, description="Parent content ID for variants")
    import_source: Optional[str] = Field(
        None, description="Import source type if created from import (e.g., 'trivie_xlsx', 'nlj_json')"
    )
    import_filename: Optional[str] = Field(None, description="Original filename if imported from file")
...
class ContentUpdate(BaseModel):
    ...
    description: Optional[str] = Field(None, description="Content description")
    nlj_data: Optional[Dict[str, Any]] = Field(None, description="Complete NLJ scenario data as JSON")
    content_type: Optional[ContentType] = Field(None, description="Type of content")
    learning_style: Optional[LearningStyle] = Field(None, description="Primary learning style for this content")
    is_template: Optional[bool] = Field(None, description="Whether this content is a template")
    template_category: Optional[str] = Field(None, max_length=100, description="Category for template organization")
...
class ContentStateUpdate(BaseModel):
    ...
    state: ContentState = Field(..., description="New content state")
    comment: Optional[str] = Field(None, max_length=500, description="Comment about the state change")
...
class ContentFilters(BaseModel):
    ...
    sort_by: Optional[str] = Field(
        default="created_at", description="Sort field: created_at, updated_at, title, view_count"
    )
