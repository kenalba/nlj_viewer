...
async def analytics_health(
    ...
    if not can_view_analytics(current_user):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="Insufficient permissions to access analytics"
        )
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Error checking analytics health: {str(e)}"
        )
...
async def get_platform_overview(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Error retrieving platform overview: {str(e)}"
        )
...
async def get_completion_stats(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Error retrieving completion stats: {str(e)}"
        )
...
async def get_learner_analytics(
    ...
    if not (current_user.email == learner_email or can_manage_users(current_user)):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="Insufficient permissions to access learner analytics"
        )

    try:
        analytics = await es_service.get_learner_analytics(learner_email=learner_email, since=since)
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Error retrieving learner analytics: {str(e)}"
        )
...
async def get_learner_statements(
    ...
    if not (current_user.email == learner_email or can_manage_users(current_user)):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="Insufficient permissions to access learner statements"
        )
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Error retrieving learner statements: {str(e)}"
        )
...
async def get_activity_analytics(
    ...
    if not can_view_analytics(current_user):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="Insufficient permissions to access activity analytics"
        )

    try:
        analytics = await es_service.get_activity_analytics(activity_id=activity_id, since=since)
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Error retrieving activity analytics: {str(e)}"
        )
...
async def get_activity_statements(
    ...
    if not can_view_analytics(current_user):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="Insufficient permissions to access activity statements"
        )
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Error retrieving activity statements: {str(e)}"
        )
...
async def get_analytics_trends(
    ...
    if not can_view_analytics(current_user):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="Insufficient permissions to access analytics trends"
        )
    ...
    try:
        ...
        overview = await es_service.get_platform_overview(since=since, until=now.isoformat())
        ...
        return {"success": True, "data": trends_data, "generated_at": datetime.now(timezone.utc).isoformat()}

    except Exception as e:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Error retrieving analytics trends: {str(e)}"
        )
...
async def get_dashboard_data(
    ...
    if not can_view_analytics(current_user):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="Insufficient permissions to access analytics dashboard"
        )
    ...
    try:
        ...
        dashboard_data = {"role": role, "platform_overview": overview}

        # Add role-specific data
        if role == "learner":
            # Get learner's personal analytics
            learner_analytics = await es_service.get_learner_analytics(learner_email=current_user.email, since=since)
        ...
        return {
            "success": True,
            "data": dashboard_data,
            "filters": {"role": role, "since": since},
            "generated_at": datetime.now(timezone.utc).isoformat(),
        }

    except Exception as e:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Error retrieving dashboard data: {str(e)}"
        )
...
async def export_analytics(
    ...
    if not can_manage_users(current_user):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="Insufficient permissions to export analytics data"
        )
    ...
    try:
        ...
        if format == "json":
            ...
            return JSONResponse(
                content={"message": "CSV export not yet implemented. Use JSON format.", "data": export_data},
                headers={
                    "Content-Disposition": f"attachment; filename=nlj_analytics_{data_type}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
                },
            )
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Error exporting analytics data: {str(e)}"
        )
...
async def get_top_performers(
    ...
    if not can_view_analytics(current_user):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="Insufficient permissions to access performance analytics"
        )

    try:
        top_performers = await performance_service.identify_top_performers(
            category=category if category != "all" else None, limit=limit, time_period=time_period
        )
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Error identifying top performers: {str(e)}"
        )
...
async def get_training_recommendations(
    ...
    if user_id and not (current_user.id == user_id or can_manage_users(current_user)):
        ...
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="Insufficient permissions to access training recommendations"
        )
    ...
    try:
        ...
        if user_id:
            ...
            user_recommendations = await performance_service.generate_training_recommendations(
                user_id=user_id, max_recommendations=limit
            )
        ...
        return {
            ...
            "data": {
                ...
                "filters": {"user_id": user_id, "category": category, "limit": limit},
            },
            "generated_at": datetime.now(timezone.utc).isoformat(),
        }
...
async def get_learner_performance_analysis(
    ...
    try:
        ...
        return {"success": True, "data": analysis_data, "generated_at": datetime.now(timezone.utc).isoformat()}
