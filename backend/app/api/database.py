...
async def get_database_status(instance_id: Optional[str] = None, current_user: User = Depends(get_current_user)):
    """
    ...
    if not can_manage_system(current_user):
        raise HTTPException(status_code=403, detail="Insufficient permissions to view database status")

    try:
        status = await rds_database_service.get_instance_status(instance_id)

        if status.get("error"):
            raise HTTPException(status_code=500, detail=f"Failed to get database status: {status['error']}")
...
async def create_snapshot(request: SnapshotRequest, current_user: User = Depends(get_current_user)):
    """
    ...
    """
    if not can_manage_system(current_user):
        raise HTTPException(status_code=403, detail="Insufficient permissions to create database snapshots")

    try:
        # Create snapshot in background for large databases
        snapshot_info = await rds_database_service.create_snapshot(
            snapshot_id=request.snapshot_id, instance_id=request.instance_id
        )
...
async def list_snapshots(
    instance_id: Optional[str] = None, max_records: int = 20, current_user: User = Depends(get_current_user)
):
    """
    ...
    """
    if not can_manage_system(current_user):
        raise HTTPException(status_code=403, detail="Insufficient permissions to list database snapshots")

    try:
        snapshots = await rds_database_service.list_snapshots(instance_id=instance_id, max_records=max_records)
...
@router.post("/restore")
async def restore_from_snapshot(request: RestoreRequest, current_user: User = Depends(get_current_user)):
    """
    ...
    """
    if not can_manage_system(current_user):
        raise HTTPException(status_code=403, detail="Insufficient permissions to restore database")
...
async def get_connection_info(instance_id: Optional[str] = None, current_user: User = Depends(get_current_user)):
    """
    ...
    """
    if not can_manage_system(current_user):
        raise HTTPException(status_code=403, detail="Insufficient permissions to view database connection info")
...
async def get_replica_info(current_user: User = Depends(get_current_user)):
    """
    ...
    """
    if not can_manage_system(current_user):
        raise HTTPException(status_code=403, detail="Insufficient permissions to view replica info")
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to get replica info: {str(e)}")
...
async def get_database_health(current_user: User = Depends(get_current_user)):
    ...
    try:
        ...
        return {"database_service": health_status, "timestamp": datetime.utcnow().isoformat()}

    except Exception as e:
        return {"database_service": {"status": "error", "error": str(e), "timestamp": datetime.utcnow().isoformat()}}
