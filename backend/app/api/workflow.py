...
from app.models import ReviewDecision, StageType, User, VersionStatus, WorkflowState, WorkflowTemplateType
...
async def create_content_version(
    request: CreateVersionRequest, current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)
):
...
async def create_approval_workflow(
    request: CreateWorkflowRequest, current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)
):
...
async def submit_for_review(
    request: SubmitForReviewRequest, current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)
):
    """Submit a content version for review."""
    try:
        workflow_service = WorkflowService(db)
        workflow = await workflow_service.submit_for_review(
            version_id=request.version_id, reviewer_id=request.reviewer_id
        )
        return WorkflowResponse.model_validate(workflow)
    except WorkflowError as e:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(e))
...
async def assign_reviewer(
    ...
    try:
        ...
        workflow = await workflow_service.assign_reviewer(
            workflow_id=workflow_id, reviewer_id=request.reviewer_id, assigner_id=current_user.id
        )
        return WorkflowResponse.model_validate(workflow)
    except WorkflowError as e:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(e))
...
async def publish_version(
    request: PublishVersionRequest, current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)
):
    """Publish a content version."""
    try:
        workflow_service = WorkflowService(db)
        version = await workflow_service.publish_version(version_id=request.version_id, publisher_id=current_user.id)
        return ContentVersionResponse.model_validate(version)
    except WorkflowError as e:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(e))
...
async def withdraw_submission(
    ...
    try:
        ...
        workflow = await workflow_service.withdraw_submission(
            workflow_id=workflow_id, creator_id=current_user.id, reason=request.reason
        )
        return WorkflowResponse.model_validate(workflow)
    except WorkflowError as e:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(e))
...
async def bulk_change_status(
    request: BulkStatusChangeRequest, current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)
):
    ...
    try:
        ...
        try:
            ...
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=f"Invalid status: {request.new_status}")

        workflow_service = WorkflowService(db)
        updated_ids = await workflow_service.bulk_change_content_status(
            content_ids=request.content_ids, new_status=new_status, user_id=current_user.id
        )
...
async def get_workflow_templates(
    ...
    try:
        workflow_service = WorkflowService(db)
        templates = await workflow_service.get_workflow_templates(content_type=content_type, is_active=is_active)
        return templates
    except WorkflowError as e:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(e))
...
async def create_multi_stage_workflow(
    ...
    try:
        ...
        workflow = await workflow_service.create_multi_stage_workflow(
            version_id=request.version_id, template_id=request.template_id, creator_id=current_user.id
        )
        return workflow
    except WorkflowError as e:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(e))
...
async def assign_stage_reviewers(
    ...
    try:
        ...
        assignments = await workflow_service.assign_stage_reviewers(
            stage_instance_id=stage_instance_id, reviewer_ids=request.reviewer_ids, assigner_id=current_user.id
        )
        return assignments
    except WorkflowError as e:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(e))
...
async def delegate_reviewer_assignment(
    request: DelegateReviewerRequest, current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)
):
...
async def get_my_stage_reviews(
    ...
    try:
        workflow_service = WorkflowService(db)
        stages = await workflow_service.get_stage_instances_for_reviewer(reviewer_id=current_user.id, states=states)
        return stages
    except WorkflowError as e:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(e))
