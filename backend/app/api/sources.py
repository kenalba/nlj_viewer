...
async def upload_source_document(
    ...
    if current_user.role not in [UserRole.CREATOR, UserRole.REVIEWER, UserRole.APPROVER, UserRole.ADMIN]:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="Insufficient permissions to upload source documents"
        )
    ...
    if len(content) > 500 * 1024 * 1024:  # 500MB
        raise HTTPException(
            status_code=status.HTTP_413_REQUEST_ENTITY_TOO_LARGE, detail="File size exceeds 500MB limit"
        )
    ...
    try:
        ...
        doc_data = {
            ...
            "original_file_type": (
                source_doc.original_file_type
                if isinstance(source_doc.original_file_type, str)
                else source_doc.original_file_type.value
            ),
            "conversion_status": (
                source_doc.conversion_status
                if isinstance(source_doc.conversion_status, str)
                else source_doc.conversion_status.value
            ),
...
async def list_source_documents(
    ...
    try:
        ...
        document_summaries = [SourceDocumentSummary.model_validate(doc) for doc in documents]

        return SourceDocumentListResponse(items=document_summaries, total=total_count, limit=limit, offset=offset)

    except Exception as e:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to list documents: {str(e)}"
        )
...
@router.get(
    "/{document_id}/download", summary="Download source document", description="Download the source document file."
)
async def download_source_document(
    ...
    return FileResponse(
        path=document.file_path, filename=document.original_filename, media_type="application/octet-stream"
    )
...
async def upload_to_claude(
    ...
    if not success:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to upload document to Claude"
        )
...
async def generate_document_summary(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to generate summary: {str(e)}"
        )
