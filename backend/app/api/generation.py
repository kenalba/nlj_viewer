...
class ContentStudioGenerateRequest(BaseModel):
    ...
    generation_type: Optional[str] = Field("scenario", description="Type of content to generate (activity, assessment, survey, etc.)")
...
async def create_generation_session(
    ...
    generate_request = GenerateContentRequest(
        ...
        prompt_config={
            **session_data.prompt_config,
            "generation_type": "scenario"  # Ensure required field is present
        },
    ...
    try:
        ...
        if result.is_completed and result.has_generated_content:
            ...
            if not generated_content:
                ...
                print(f"  - Keys (if dict): {list(generated_content.keys()) if isinstance(generated_content, dict) else 'N/A'}")
        ...
        print(f"📤 Returning API response:")
        print(f"  - Status: {response.status}")
        print(f"  - Progress: {response.progress_percentage}%")
        print(f"  - Generated content included: {bool(response.generated_content)}")
        if response.generated_content:
            print(f"  - Generated content size: {len(str(response.generated_content))}")
        ...
        try:
            ...
            debug_info = {
                ...
                "validated_nlj": {
                    ...
                    "node_count": len(session.validated_nlj.get('nodes', [])) if isinstance(session.validated_nlj, dict) else 0,
                    "link_count": len(session.validated_nlj.get('links', [])) if isinstance(session.validated_nlj, dict) else 0,
                ...
                "timing": {
                    "total_duration_seconds": (
                        (session.completed_at or session.failed_at or datetime.now(timezone.utc)) - session.created_at
                    ).total_seconds() if session.created_at else None,
                    "generation_duration_seconds": (
                        (session.completed_at or session.failed_at) - session.started_at
                    ).total_seconds() if session.started_at and (session.completed_at or session.failed_at) else None
                }
            }
            ...
            raise HTTPException(
                status_code=500,
                detail=f"Failed to retrieve debug session details: {str(e)}"
            )
