...
from app.services.email_service import EmailRecipient, email_service
...
async def send_survey_invitation(
    request: SurveyInvitationRequest, current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)
):
    """
    ...
    if not settings.ENABLE_EMAIL_NOTIFICATIONS:
        raise HTTPException(
            status_code=503, detail="Email notifications are currently disabled. Please contact your administrator."
        )
    ...
    try:
        ...
        recipients = [
            EmailRecipient(
                email=r.email, name=r.name, user_id=None  # External survey recipients don't have user accounts
            )
        ...
        response = EmailResponse(
            ...
            message=(
                f"Survey {'reminder' if request.reminder else 'invitation'} sent successfully"
                if result.success
                else f"Failed to send emails: {result.error_message}"
            ),
        ...
        if not result.success and result.failed_recipients:
            ...
            response.message = (
                f"Emails sent to {result.recipient_count} recipients, {len(result.failed_recipients)} failed"
            )

        return response

    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Failed to process survey invitation request: {str(e)}")
...
async def send_training_notification(
    ...
    """
    if not settings.ENABLE_EMAIL_NOTIFICATIONS:
        raise HTTPException(status_code=503, detail="Email notifications are currently disabled")

    try:
        # Convert request recipients to service recipients
        recipients = [EmailRecipient(email=r.email, name=r.name) for r in request.recipients]
        ...
        return EmailResponse(
            success=result.success,
            message=(
                "Training notification sent successfully"
                if result.success
                else f"Failed to send notifications: {result.error_message}"
            ),
            recipient_count=result.recipient_count,
            failed_recipients=result.failed_recipients,
            message_id=result.message_id,
        ...
        raise HTTPException(status_code=500, detail=f"Failed to send training notification: {str(e)}")
...
async def send_admin_notification(
    ...
    try:
        ...
        result = await email_service.send_admin_notification(
            subject=subject, message=message, notification_type=notification_type, additional_data=additional_data
        )
        ...
        raise HTTPException(status_code=500, detail=f"Failed to send admin notification: {str(e)}")
...
async def get_email_service_health():
    ...
    try:
        ...
        return {
            "email_service": {"status": "error", "error": str(e), "timestamp": datetime.utcnow().isoformat()},
            "notifications_enabled": settings.ENABLE_EMAIL_NOTIFICATIONS,
        }
