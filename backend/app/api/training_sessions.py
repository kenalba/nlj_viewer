...
async def create_training_program(
    ...
    try:
        ...
        await db.rollback()
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=f"Program creation error: {str(e)}")
    except Exception as e:
        logger.error(f"Error creating training program: {e}")
        await db.rollback()
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to create training program"
        )
...
async def get_training_session(
    ...
    stmt = (
        select(TrainingSession).options(selectinload(TrainingSession.bookings)).where(TrainingSession.id == session_id)
    )
    ...
    confirmed_bookings = len([b for b in (session.bookings or []) if b.booking_status in ["confirmed", "pending"]])
    waitlist_bookings = len([b for b in (session.bookings or []) if b.booking_status == "waitlist"])
...
async def list_training_sessions(
    ...
    stmt = (
        ...
        .options(selectinload(TrainingSession.bookings), selectinload(TrainingSession.program))
...
async def update_training_session(
    ...
    if not check_session_permissions(current_user, "update"):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="Insufficient permissions to update training sessions"
        )
    ...
    if current_user.role not in [UserRole.ADMIN, UserRole.APPROVER] and session.created_by_id != current_user.id:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Can only update your own training sessions")
    ...
    try:
        ...
        await db.rollback()
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=f"Scheduling error: {e.message}")
    except Exception as e:
        logger.error(f"Error updating training session: {e}")
        await db.rollback()
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to update training session"
        )
...
async def delete_training_session(
    ...
    if not check_session_permissions(current_user, "delete"):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="Insufficient permissions to delete training sessions"
        )
    ...
    # Check ownership for non-admin users
    if current_user.role not in [UserRole.ADMIN, UserRole.APPROVER] and session.created_by_id != current_user.id:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Can only delete your own training sessions")

    # Check for existing bookings
    stmt = select(TrainingBooking).where(
        and_(TrainingBooking.session_id == session_id, TrainingBooking.booking_status.in_(["confirmed", "pending"]))
    )
    result = await db.execute(stmt)
    ...
    try:
        ...
        await db.rollback()
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to delete training session"
        )
