...
class FakeDataGenerator:
    ...
    async def initialize(self):
        ...
        result = await self.session.execute(
            select(ContentItem).where(ContentItem.state == "published")
        )
    ...
    async def create_additional_fake_users(self, count: int = 50) -> List[User]:
        ...
        first_names = [
            ...
            "Sam", "Devon", "Sage", "River", "Skylar", "Rowan", "Phoenix", "Finley",
            "Blake", "Cameron", "Drew", "Emery", "Jamie", "Kai", "Lane", "Parker",
            "Reese", "Shay", "Toni", "Val", "Wren", "Zion", "Arden", "Bay",
            "Cedar", "Dani", "Ellis", "Frankie", "Gray", "Haven", "Indigo", "Jules",
            "Kris", "Lennox", "Max", "Nova", "Ocean", "Peyton", "Quincy", "Remy"
        ]
    ...
    def simulate_learning_session(
        ...
        statements.append(self.create_xapi_statement(
            user, activity, 'experienced', current_time
        ))
        ...
        if activity.nlj_data and 'nodes' in activity.nlj_data:
            question_nodes = [
                node for node in activity.nlj_data['nodes']
                if node.get('type') not in ['start', 'end', 'panel']
            ]
            ...
            for i, node in enumerate(question_nodes):
                ...
                statements.append(self.create_xapi_statement(
                    user, activity, 'attempted', current_time,
                    context={
                        "extensions": {
                ...
                if success:
                    ...
                    statements.append(self.create_xapi_statement(
                        user, activity, 'answered', current_time,
                        result={
                            "success": True,
                            "score": {"scaled": score},
        ...
        if random.random() < completion_rate:
            ...
            statements.append(self.create_xapi_statement(
                user, activity, 'completed', current_time,
                result={
                    "completion": True,
            ...
            if overall_score >= 0.7:
                statements.append(self.create_xapi_statement(
                    user, activity, 'passed', current_time,
                    result={
                        "success": True,
                        "score": {"scaled": overall_score}
                    }
                ...
                statements.append(self.create_xapi_statement(
                    user, activity, 'failed', current_time,
                    result={
                        "success": False,
                        "score": {"scaled": overall_score}
                    }
        ...
        for day in range(days):
            ...
            for user in users_to_use:
                ...
                activities_this_session = random.choices(
                    self.activities,
                    k=random.randint(1, min(3, len(self.activities)))
                )
        ...
        try:
            ...
            for i in range(0, len(self.generated_events), batch_size):
                ...
                if (i // batch_size + 1) % 10 == 0:  # Progress every 10 batches
                    print(f"  📦 Sent {min(i + batch_size, len(self.generated_events))}/{len(self.generated_events)} events")
    ...
    try:
        async with db_manager.get_session() as session:
            ...
            if stream_to_kafka:
                ...
                print(f"  • Realistic completion rates and scores")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--preset", choices=["demo", "dev", "full"], 
