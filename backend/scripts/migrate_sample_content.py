...
class ContentMigrator:
    ...
    async def clear_existing_content(self, session):
        ...
        if not self.dry_run:
            ...
            result = await session.execute(select(ContentItem).where(ContentItem.is_template))
    ...
    def extract_content_metadata(self, content_data: Dict[str, Any], file_path: Path) -> Dict[str, Any]:
        ...
        has_interactive = any(
            node.get("type") in ["choice", "matching", "ordering", "connections", "wordle"] for node in nodes
        )
        has_text_heavy = any(len(str(node.get("text", ""))) > 500 for node in nodes)
    ...
    async def migrate_json_content(self, session, file_path: Path, admin_user_id: uuid.UUID):
        ...
        try:
            ...
            existing = await session.execute(
                select(ContentItem).where(ContentItem.title == metadata["title"], ContentItem.is_template)
            )
    ...
    async def create_database_backup(self):
        ...
        async with db_manager.get_session() as session:
            try:
                ...
                backup_data = {"timestamp": timestamp, "content_count": len(content_items), "content": []}
...
async def main():
    ...
    parser.add_argument("--force", action="store_true", help="Delete existing sample content before migration")
    parser.add_argument("--backup", action="store_true", help="Create database backup after migration")
